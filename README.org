...
2. If you don't have a public/private key pair, generate them
   ...
   #+END_SRC
3. Set permissions for directory and keys
   #+BEGIN_SRC bash
   cd
...
2. Emacs
** Developer Tools
The "Developer Tools" tab will appear when you run the command to enable developer mode for the terminal (1)
1. Terminal
   #+BEGIN_SRC bash
...
* Shell configuration
Clone the dot file repository https://github.com/devnich/dotfiles into ~~/dotfiles~ and follow the installation instructions in the README. Note that some files (e.g., ~.zshenv~, ~.nix-channel~) may conflict with files you have already created during the setup process. If this happens, you should merge the contents of each "live" file into its respective repository version before running the ~stow~ command.
